/*
 * Code originally gotten from: https://github.com/sharase/melg-64/blob/dd12a55829b102b396146e4fb73543ebb368ec05/melg19937-64/melg19937-64.c
 * 
 * Code modified to always use uint64_t istead of ull and results are reversed
 * because then the least-significant bits have more spread.
 */
...
#define NN 311 // N-1
#define MM 81 // M
#define MATRIX_A (uint64_t)0x5c32e06df730fc42
#define P 33 // W-r
#define W 64
#define MASKU ((uint64_t)0xffffffffffffffff << (W-P))
#define MASKL (~MASKU)
#define MAT3NEG(t, v) (v ^ (v << ((t))))
#define MAT3POS(t, v) (v ^ (v >> ((t))))
#define LAG1 19 // L
#define SHIFT1 16 // s_3
#define MASK1 (uint64_t)0x6aede6fd97b338ec // b
#define LAG1over 292 // NN-LAG1

static uint64_t melg[NN]; 
static int melgi;
static uint64_t lung; //extra state variable
static uint64_t mag01[2]={(uint64_t)0, MATRIX_A};
static uint64_t x;

static uint64_t case_1(void);
...
void init_genrand64(uint64_t seed)
{
    ...
    for (melgi=1; melgi<NN; melgi++) {
        melg[melgi] = ((uint64_t)6364136223846793005 * (melg[melgi-1] ^ (melg[melgi-1] >> 62)) + melgi);
    }
    lung = ((uint64_t)6364136223846793005 * (melg[melgi-1] ^ (melg[melgi-1] >> 62)) + melgi);
    melgi = 0;
...
{
	uint64_t i, j, k;
    init_genrand64((uint64_t)19650218);
    i=1; j=0;
    k = (NN>key_length ? NN : key_length);
    for (; k; k--) {
        melg[i] = (melg[i] ^ ((melg[i-1] ^ (melg[i-1] >> 62)) * (uint64_t)3935559000370003845))
    ...
    for (k=NN-1; k; k--) {
        melg[i] = (melg[i] ^ ((melg[i-1] ^ (melg[i-1] >> 62)) * (uint64_t)2862933555777941757))
          - i; /* non linear */
        i++;
        if (i>=NN) { melg[0] = melg[NN-1]; i=1; }
    }
    lung = (lung ^ ((melg[NN-1] ^ (melg[NN-1] >> 62)) * (uint64_t)2862933555777941757))
	  - NN; /* non linear */
    melg[0] = (melg[0] | ((uint64_t)1 << 63)); /* MSB is 1; assuring non-zero initial array. Corrected.  */
    melgi = 0;
}

static uint64_t case_1(void) {
    x = (melg[melgi] & MASKU) | (melg[melgi+1] & MASKL);
    lung = (x >> 1) ^ mag01[(int)(x & (uint64_t)1)] ^ melg[melgi+MM] ^ MAT3NEG(23, lung);
...
static uint64_t case_2(void) {
    x = (melg[melgi] & MASKU) | (melg[melgi+1] & MASKL);
    lung = (x >> 1) ^ mag01[(int)(x & (uint64_t)1)] ^ melg[melgi+(MM-NN)] ^ MAT3NEG(23, lung);
    melg[melgi] = x ^ MAT3POS(33, lung);
    x = melg[melgi] ^ (melg[melgi] << SHIFT1);
    x = x ^ (melg[melgi + LAG1] & MASK1);
    ++melgi;
...
static uint64_t case_4(void) {
    ...
    lung = (x >> 1) ^ mag01[(int)(x & (uint64_t)1)] ^ melg[MM-1] ^ MAT3NEG(23, lung);
